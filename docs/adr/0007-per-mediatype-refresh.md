...
- **Video refresh is ad-complete-driven (superseded trigger, D66).** Banner/native use the time-based `RefreshScheduler` (`intervalSec`). Video does **not** — its `refresh` accepts `sessionCap` only (`intervalSec` is rejected with a `ConfigError`), and the refresh is triggered by the video finishing — IMA `COMPLETE` (`adComplete`) or user `SKIPPED` (`adSkipped`) — via `armVideoRefresh`, re-auctioning on each up to `sessionCap` (completions and skips share the counter). (The timer path never ran for video anyway: the video branch of `onAuctionWon` returns before the `viewable → startRefreshIfConfigured` block.)
