...
5. **Execution context = publisher top page**. SDK runs directly on the publisher's document, not inside an ad-server creative iframe. Trade-off: the example HTML's `meta name="ad.size"` is treated as illustrative and unused. **Amended by D65 (surface-aware):** the SDK may also run *inside* a GAM creative iframe (`friendly-iframe` or `safeframe`) as a GAM-unfill backfill, detecting its surface and degrading accordingly.
...
64. **Refresh config moved from slot-root to per-mediaType** — relocates the `refresh` field (D17) from `ValidatedSlotConfig` into each `mediaTypes.<format>` (`banner.refresh`, `video.refresh`, `native.refresh`); reverses the slot-root placement in D42. **Cadence follows the rendered mediaType:** a slot renders one winning creative at a time (highest CPM across formats), so `SlotLifecycle` records the winning bid's mediaType (`onAuctionWon`) and reads that format's `refresh` for both the refresh scheduler and the adComplete gate. **Re-evaluated each impression:** because a refreshed auction can hand a different format the win, the scheduler's interval is retuned per impression via `RefreshScheduler.updateInterval(ms)` — which resets the countdown to the new interval from now while preserving the session-cap fire count (so a mediaType switch never resets the cap). If the rendered mediaType has no `refresh`, the slot stops refreshing while that creative shows (a running scheduler is cancelled). Validation (min-interval floor from `minRefreshIntervalSec`, sessionCap ≥ 1) is unchanged, just applied per-mediaType with `mediaTypes.<format>.refresh.*` error fields. Trade-off: a slot with mixed formats can change refresh rate between impressions and can go dormant if a no-refresh format wins — intended, since cadence is a property of what's actually on screen. **Video refresh is ad-complete-driven, not timer-driven — see D66.** See `docs/adr/0007-per-mediatype-refresh.md`.
65. **Execution surface is detected; capabilities degrade per surface** — amends D5 (top-page-only) and extends the D30/D34 environment→degradation pattern. Enables a **GAM-first waterfall backfill**: GAM serves first, and on **unfill** serves an HTML5 creative that loads the SDK (`gen_ad.min.js`), which runs the Prebid-direct auction and renders the winner *inside the creative iframe*. The SDK detects its surface — `top` (`window===window.top`), `friendly-iframe` (framed, same-origin, `window.top.document` reachable), or `safeframe` (framed, cross-origin / `$sf.ext` present) — orthogonal to the existing `webview` axis. **Degradation:** identity is **OFF in `safeframe`** (browser-enforced — cross-origin storage partitioning + 3p-cookie blocking make Prebid cookies / first-party ids / vendor-cookie reads unusable; mirrors webview D34), ON in `top`/`friendly-iframe`; consent is wired per-surface — `window.__tcfapi` (top) / `window.top.__tcfapi` (friendly) / IAB `__tcfLocator` postMessage bridge (safeframe) — and gated by `consentDisabled` (false → SDK gate active per D15; true → SDK gate skipped, Prebid's `consentManagementTcf` still runs); viewability uses IntersectionObserver (top/friendly) / `$sf.ext.geom()` (safeframe); render is banner on all surfaces, in+outstream video on `top`, outstream on `friendly-iframe`, **best-effort** outstream on `safeframe` (on outstream failure in safeframe, fall back to the highest banner bid from the same auction, else `noFill`). **Refresh stays SDK-owned and enabled on all browser surfaces incl. framed** (the SDK rotates its own backfill in the GAM-given slot; GAM + SDK may both refresh, no stacking since the container is cleared first) — webview keeps refresh OFF (D34); consequence: `safeframe` refresh requires the `$sf.ext` viewability provider (IO is blind cross-origin). Topology is **backfill, not unified header bidding** — GAM keeps first-look; "highest CPM wins" (Prebid→GPT→GAM) remains the deferred v2 item, D9. See `docs/adr/0008-execution-surface-detection.md`.
66. **Video refresh is ad-complete-driven, not timer-driven** — refines D64. Banner/native refresh stays time-based (`mediaTypes.<format>.refresh.intervalSec` + `RefreshScheduler`), but a **video** mediaType's refresh is triggered by the ad finishing: `mediaTypes.video.refresh` accepts **`sessionCap` only** — `intervalSec` is rejected with a `ConfigError` (it was meaningless for video, and the timer path never even ran for video because `onAuctionWon`'s video branch returns before arming the scheduler). On the video finishing — IMA `COMPLETE` (`adComplete`) **or** user `SKIPPED` (`adSkipped`), both `{mediaType:"video"}` — `SlotLifecycle.armVideoRefresh` re-auctions the slot (full re-auction, D64) and re-arms on the next video render, bounded by `sessionCap` (completions + skips share one counter; then `refresh_cap_reached`). Render errors do not trigger it. Respects `suppressRefresh` (webview OFF, D34). Trade-off: video can't be refreshed on a wall-clock while playing (correct — never interrupt playback); a completed video with no `sessionCap` re-auctions on every completion until out of view / destroyed. See `docs/adr/0007-per-mediatype-refresh.md`.
67. **Every video adUnit gets an inert `adUnit.renderer` stub — never a real render path** — fixes an outstream-video demand gap: Prebid.js core strips `mediaTypes.video` before `requestBids` for any bidder without a renderer/`outstreamAU` present (confirmed core-level in the vendored bundle, not PubMatic-specific), so only PubMatic (via our existing `outstreamAU` injection) could serve outstream; Rubicon and the rest of the v1 bidder set (D8) could not. Fix is **not** a publisher-configurable renderer (rejects the field-request's Option A) — `.renderer.render(` has zero call sites in the vendored Prebid bundle, so a renderer's `render()` callback is never invoked by this codebase; real playback stays exclusively on `VideoRenderer`/IMA (D1/D41) regardless of winning bidder. `src/core/prebid-outstream-stub.ts` exports one shared, stateless stub object (`render` is a no-op, no `url` — never fetched, since Prebid only loads a renderer's `url` inside `Renderer.render()`); `auction-orchestrator.ts`'s adUnit builder attaches it at adUnit root (not nested under `mediaTypes.video`) whenever `mediaTypes.video` is present, unconditionally regardless of declared `context` (D41: instream-declared video still renders as outstream). Deliberately placed outside `src/renderers/` — it isn't an instance of the SDK's own `.render(args)` renderer abstraction, just a Prebid-facing presence-check object. Trade-off: looks like dead code (nothing calls it) and is tempting to "clean up" — doing so silently reverts non-PubMatic outstream bidders to banner-only; see `docs/adr/0009-inert-outstream-renderer-stub.md`. Does not address the separately-discovered `plcmt`/`maxduration` schema gap in `VideoMediaType` (tracked as a follow-up, different root cause).
