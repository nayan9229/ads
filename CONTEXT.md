...
- **Prebid modules** (original v1 intent — the *built* set is narrower, see **D62**): `consentManagementTcf`, `consentManagementUsp`, `gdprEnforcement` (→ `tcfControl` in Prebid 9), ~~`priceFloors`~~ (dropped — static config floors, D10), `currency`, `userId` (sharedId + id5Id + optional uid2 submodules), ~~`dfpAdServerVideo`~~ (dropped — no GAM, D9).
